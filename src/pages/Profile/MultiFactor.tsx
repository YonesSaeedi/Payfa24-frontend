...
export default function MultiFactor() {
  ...
  const handleCardClick = (type: string, isActive: boolean) => {
    ...
    if (!isActive) {
      ...
      try {
        ...
        toast.error((error as AxiosError<{ msg?: string }>)?.response?.data?.msg || "در ارسال کد مشکلی پیش آمد.");
      } finally {
  ...
  const handleResendCode = async () => {
    try {
      ...
      toast.error((error as AxiosError<{ msg?: string }>)?.response?.data?.msg || "در ارسال دوباره کد مشکلی پیش آمد.");
    } finally {
      setIsSubmittingResendCode(false);
    }
  };
    ...
    <>
                  ...
                  <div className="w-full flex gap-4 flex-col lg:mt-10 mt-6 mb-20">
                    {MultiInfo.map((item) => {
                      ...
                      return (
                        <MultiFactorCard
                          key={item.type}
                          dataCard={{ ...item, button: isActive ? "غیرفعال کردن" : "فعال کردن" }}
      ...
      {isOpenActive && (
        <OTPInputModal
          ...
          mainText={
            twoFAData?.twofa?.type === "email"
              ? `لطفا کد ارسال شده به ایمیل ${userData?.user?.email} را وارد کنید`
              : twoFAData?.twofa?.type === "google"
              ? "لطفا کد ورود دو مرحله‌ای گوگل را فعال کنید."
              : twoFAData?.twofa?.type === "sms"
              ? `لطفا کد ارسال شده به پیامک ${userData?.user?.mobile} را وارد کنید.`
