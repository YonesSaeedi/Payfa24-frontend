...
//       return { code: code || "", email: trimmed, recaptcha } as Record<string, string>;
//     if (iranMobileRegex.test(trimmed))
//       return { code: code || "", mobile: trimmed, recaptcha } as Record<string, string>;
...
export default function ForgotPasswordPage() {
  ...
  const { handleSubmit, control, formState: { errors }, getValues, watch } = useForm<FormData>({
    resolver: yupResolver(schema),
    defaultValues: { mobile: "" }, // تغییر به mobile
  });

  const mobileValue = watch("mobile"); // تغییر به mobile
