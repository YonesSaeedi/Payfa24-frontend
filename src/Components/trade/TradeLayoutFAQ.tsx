...
interface TradeLayoutFAQProps {
  items: FAQItem[];
   openMaxHeightLg?: string;   

  closedMaxHeight?: string;    
}

const TradeLayoutFAQ: React.FC<TradeLayoutFAQProps> = ({ items,openMaxHeightLg = "lg:max-h-40",closedMaxHeight = "max-h-0" }) => {
        ...
        >
          <div
            className={`flex items-center justify-between ${
              data.id === openQuestionID ? "border-b border-borderSecondary lg:pb-4 pb-3" : ""
            }`}
          >
            <p className="font-normal text-sm lg:text-base xl:text-lg text-black1">
              {data.question}
            </p>
          ...
          <p
  className={`text-xs lg:text-sm xl:text-base font-normal text-gray5 transition-all duration-300 ease-out ${
    data.id === openQuestionID
      ? `${openMaxHeightLg}  opacity-100 mt-3 lg:mt-4`
      : `${closedMaxHeight} opacity-0 mt-0 lg:mt-0`
  }`}
>
  {data.answer}
</p>
