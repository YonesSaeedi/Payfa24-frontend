...
const BreadcrumbNavigation = () => {
  ...
  return (
    <nav
      className="w-full flex items-center gap-1 lg:gap-2 text-xs lg:text-sm font-normal"
      dir="rtl"
    >
      <Link
        className="text-text2 text-black0 hover:text-blue2 hover:underline"
        to="/"
      >
        پی‌فا24
      </Link>
      {pathnames.map((value, index) => {
        ...
        return (
          <React.Fragment key={to}>
            <span className="w-4 h-4 text-text2 text-black0">
              <IconChevron />
            </span>
            {isLast ? (
              <span className="text-blue2">{BREADCRUMB_LABELS[value]}</span>
              ...
              <Link
                className="text-black1 hover:text-blue2 hover:underline"
                to={to}
              >
