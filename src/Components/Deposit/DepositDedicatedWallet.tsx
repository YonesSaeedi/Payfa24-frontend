...
export default function DepositDedicatedWallet() {
  ...
  const networkOptions = useMemo(() => {
    return (
      selectedCurrency.network?.map((net) => {
        const networkInfo = networks.find((n) => n.id === net.id);
        const networkName =
          networkInfo?.locale?.fa?.name || networkInfo?.name || net.id.toString();
        const networkSymbol = networkInfo?.symbol || networkInfo?.name || net.id.toString();
        return {
          value: net.id.toString(),
  ...
  // دریافت و نگاشت داده‌های ارزها با مدیریت بهتر آیکون‌ها
  const fetchAndMergeCryptoData = useCallback(async () => {
    setIsDepositCoinsLoading(true);
    try {
      const depositRes = await apiRequest({
      ...
      const merged: CryptoItem[] = depositCoins.map((coin: any) => {
        ...
        // اگر info پیدا نشد، از symbol برای ساخت fallback آیکون استفاده می‌کنیم
        const fallbackIcon = `${coin.symbol.toLowerCase()}.png`;
        return {
          id: coin.id,
          symbol: coin.symbol || "UNK",
          name: info?.locale?.fa?.name || coin.symbol || "نام ناشناس",
          icon: info?.icon || fallbackIcon, // اگر icon نباشد، از symbol-based fallback
          priceBuy: (Number(coin.price) || 0).toString(),
          balance: (Number(coin.balance) || 0).toString(),
          isFont: info?.isFont || false,
          color: info?.color || "#000",
  ...
  const handleCurrencySelect = (
    ...
    setSelectedCurrency({
      ...
      isFont: crypto.isFont || false,
      color: crypto.color || "#000",
      icon: crypto.icon || `${crypto.symbol.toLowerCase()}.png`, // fallback
      network: crypto.network || [],
    });
    setValue("network", ""); // ریست شبکه هنگام تغییر ارز
    setWalletAddress(null); // ریست آدرس
    setShowWalletInfo(false); // مخفی کردن اطلاعات کیف پول
    setIsCryptoListModalOpen(false);
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.error("خطا در دریافت آدرس:", err);
      toast.error("خطا در دریافت آدرس ولت");
      setShowWalletInfo(false);
    }
  };
          ...
          {selectedCurrency?.id ? (
            <span className="flex items-center gap-2">
              {selectedCurrency.isFont ? (
                <i
                  className={`cf cf-${selectedCurrency.symbol?.toLowerCase()}`}
                  style={{ color: selectedCurrency.color || "#000", fontSize: "24px" }}
                />
              ) : (
                <img
                  src={`https://api.payfa24.org/images/currency/${selectedCurrency.icon || `${selectedCurrency.symbol?.toLowerCase()}.png`}`}
                  alt={selectedCurrency.symbol || "UNK"}
                  className="w-6 h-6 object-contain"
                  onError={(e) => {
                    e.currentTarget.src = `https://api.payfa24.org/images/currency/btc.png`; // fallback در صورت خطا
                  }}
                />
              )}
              <span className="text-black1 font-medium">{selectedCurrency.symbol}</span>
            </span>
          ) : (
        ...
        render={({ field }) => (
          <div className="relative">
            <FloatingSelect
              ...
              onChange={(value) => {
                field.onChange(value);
                setWalletAddress(null); // ریست آدرس هنگام تغییر شبکه
                setShowWalletInfo(false);
              }}
              onOpen={() => setIsNetworkDropdownOpen(!isNetworkDropdownOpen)}
              placeholderClasses="text-black0"
            />
            {isNetworkDropdownOpen && (
              <div
              ...
              >
                {networkOptions.map((option) => (
                  <label
                    key={option.value}
                    className={`flex items-center justify-end gap-3 p-3 flex-row-reverse rounded-md transition-colors w-full cursor-pointer ${
                      field.value === option.value ? "bg-gray-100 text-blue-600" : ""
                    }`}
                    onClick={(e) => {
                      e.stopPropagation();
                      field.onChange(option.value);
