...
// export default function StepCard({ onNext, userInfo, refetch }: Props) {
//   const [isModalOpen, setIsModalOpen] = useState(false);
...
//   // اگر قبلاً کارت ثبت شده بود → برو مرحله بعد
//   useEffect(() => {
//     if (userInfo?.kyc?.basic?.cardbank) {
//       onNext(); // مستقیم برو مرحله بعد
//     }
//   }, [userInfo, onNext]);
...
//       if (data.status) {
//         toast.success("شماره کارت با موفقیت ثبت شد.");
//         await refetch(); // آپدیت داده‌ها
//         setIsModalOpen(true); // فقط بعد از موفقیت
//       } else {
//         toast.error(data.msg || "خطا در ثبت شماره کارت.");
//         onNext(); // حتی اگر خطا بود، برو جلو (مثل بقیه)
//       }
//     },
//     onError: (error: AxiosError<{ msg: string }>) => {
//       const errorMsg = error.response?.data?.msg || "خطا در ارتباط با سرور.";
//       toast.error(errorMsg);
//       onNext(); // حتی در خطا هم برو مرحله بعد
//     },
//   });
...
//   const handleSkip = () => {
//     onNext(); // دکمه "بعداً انجام می‌دم" → فقط برو مرحله بعد
//   };

//   const handleCloseModal = () => {
//     setIsModalOpen(false);
//     // مودال فقط بعد از موفقیت باز میشه → بعد از بسته شدن می‌تونه بره به پیشرفته
...
//           <p className="lg:text-xl text-sm font-medium mt-10 lg:mb-8 mb-5 text-end text-black0">
...
//                     field.onChange(pasted);
//                     const input = e.target as HTMLInputElement;
//                     input.value = toPersianDigits(formatCardNumber(pasted));
//                   }}
//                   placeholder="____-____-____-____"
//                   inputMode="numeric"
//                   className="input-card text-center tracking-widest"
...
//         {/* مودال موفقیت — فقط بعد از ارسال موفق */}
//         {isModalOpen && (
//           <div>
//             <div className="fixed inset-0 bg-black bg-opacity-50 z-45"></div>
