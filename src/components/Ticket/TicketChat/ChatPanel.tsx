...
const ChatPanel: React.FC<ChatPanelProps> = ({ ticket }) => {
  ...
  useEffect(() => {
    ...
    ticketInfo.ticket_message.forEach((msg) => {
      ...
      fetchFileAsDataUrl(msg.file)
        .then((fileUrl) => {
          if (fileUrl) {
            const next = { ...imageCacheRef.current, [id]: fileUrl };
  ...
  const handleSend = async (e?: React.MouseEvent<HTMLButtonElement>) => {
    ...
    try {
      ...
      // ---------- حذف کامل get-info اضافی ----------
      if (selectedFile && tempUrl) {
        // دانلود مستقیم فایل و قرار دادن در cache
        fetchFileAsDataUrl(selectedFile.name).then((fileUrl) => {
          if (fileUrl) {
