...
const TransactionModalFiat: React.FC<TransactionModalFiatProps> = ({ tx, onClose }) => {
  ...
  const renderIcon = (size = "w-20 h-20") => (
    <img src={ImageIran} alt="تومان" className={`${size} rounded-full object-cover`} />
  );
          ...
          {loading ? (
            ...
            <>
              <div className="flex flex-col items-center mb-10">
                {renderIcon()}
                <h3 className="font-medium text-[24px] mt-3 text-black1">تومان</h3>
                <span className="text-sm text-gray-500 dark:text-gray-400">IRT</span>
              </div>
              ...
              <div className="grid grid-cols-1 gap-6 text-sm">
                ...
                {detail.stock && (
                  <DetailRow
                    ...
                    value={formatPersianNumber(detail.stock)}
                    symbol="تومان"
                  />
                )}
