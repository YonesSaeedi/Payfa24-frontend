    ...
    const fetchOrderDetail = async () => {
      ...
      try {
        ...
        if (res.order) {
          ...
          const matchedCoin = mappedGeneralData[tx.symbol || ""];

          const mergedCoin: OrderDetail["coin"] = {
            name: matchedCoin?.name ?? res.order.coin?.name,
            faName: matchedCoin?.locale?.fa?.name ?? res.order.coin?.faName ?? res.order.coin?.name,
            symbol: matchedCoin?.symbol ?? res.order.coin?.symbol,
            icon: matchedCoin?.icon ?? res.order.coin?.icon ?? "/images/fallback-coin.png",
            isFont: matchedCoin?.isFont ?? res.order.coin?.isFont ?? false,
          };
