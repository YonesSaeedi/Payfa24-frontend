...
const CryptoPage: React.FC = () => {
  ...
  const [selectedFilterFor, setSelectedFilterFor] = useState(
    filterForOptions[0]
  );
          ...
          <FilterDropdown
            ...
            onSelect={(name) =>
              setSelectedFilterType(
                typeOptions.find((o) => o.name === name) || typeOptions[0]
              )
            }
          />
          <FilterDropdown
            ...
            onSelect={(name) =>
              setSelectedStatus(
                statusOptions.find((o) => o.name === name) || statusOptions[0]
              )
            }
          />
          <FilterDropdown
            ...
            onSelect={(name) =>
              setSelectedFilterFor(
                filterForOptions.find((o) => o.name === name) ||
                  filterForOptions[0]
              )
            }
          />
          ...
          {isLoading ? (
            ...
            <div className="divide-y divide-gray21">
              {filteredTransactions.map((tx) => (
                <div
                  key={tx.id}
                  className="grid grid-cols-6 py-4 px-3 hover:bg-gray42 items-center text-sm"
                >
                  <div className="flex items-center gap-2">
                    <span className="w-10 h-10 flex items-center justify-center object-cover">
                      {tx.isFont ? (
                        <i
                          className={`cf cf-${tx.coin.symbol?.toLowerCase()}  w-10 h-10 size-10 text-[35px]`}
                    ...
                    <div className="flex flex-col gap-1 text-right">
                      <span className="font-normal text-lg">
                        {tx.locale?.fa?.name || tx.coin.name}
                      </span>
                  ...
                  <div className="text-center">
                    <StatusBadge
                      text={transactionStatusMap[tx.status] || "نامشخص"}
                    />
                  </div>
                  <div className="text-center font-normal text-base">
                    {tx.DateTime}
          ...
          {filteredTransactions.length > 0 ? (
            filteredTransactions.map((tx) => (
              <div key={tx.id} className="border rounded-xl p-4  border-gray21">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    <span className="w-10 h-10 flex items-center justify-center object-cover">
                      {tx.isFont ? (
                        <i
                          className={`cf cf-${tx.coin.symbol?.toLowerCase()} text-[28px]`}
                          style={{ color: tx.color ?? "" }}
                        />
                      ) : (
                        <img
                ...
                <div
                  className="text-blue-600 text-sm mt-3 cursor-pointer border-t pt-3 border-gray21  text-center font-bold text-[14px]"
                  onClick={() => handleOpenModal(tx)}
                >
                  جزئیات تراکنش
                </div>
