...
const skeletonArray: number[] = Array.from({ length: 5 }, (_, i) => i);

  return (
    <div className="bg-gray27 rounded-2xl p-4 w-full ">
  ...
  <ul className="grid grid-cols-3 gap-y-2 pt-2 pb-4" dir="rtl">
  {isLoading
    ? skeletonArray.map((_, index) => (
        <li key={index} className="contents animate-pulse">
    ...
    : items.slice(0, 5).map((item, index) => (
        <li key={`${item.symbol}-${index}`} className="contents">
          <div className="flex items-center gap-2">
            ...
            <div className="flex flex-col text-right leading-tight">
              <span className="text-black1 mb-1/2 font-normal text-base whitespace-nowrap">
